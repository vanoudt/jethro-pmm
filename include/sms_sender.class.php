    ...
    public static function sendMessage($message, $recips, $saveComm = TRUE)
    {
        ...
        if (FALSE !== strpos($content, '_USER_MOBILE_')) {
            ...
            if (!strlen($me->getValue('mobile_tel'))) {
                return array(
                    'success' => FALSE, 'successes' => array(), 'failures' => array(), 'rawresponse' => '',
        ...
        if (
            strlen(ifdef('SMS_LOCAL_PREFIX')) && strlen(ifdef('SMS_INTERNATIONAL_PREFIX')) && FALSE !== strpos(SMS_HTTP_POST_TEMPLATE, '_RECIPIENTS_INTERNATIONAL')
        ) {
            $intls = array();
        ...
        ///HACK TO WORK WITH JSON FOR CELLCAST
        $cellcast_message = $message;
        $cellcast_numbers = $mobile_tels;
        if (strlen(ifdef('SMS_LOCAL_PREFIX')) && strlen(ifdef('SMS_INTERNATIONAL_PREFIX')) && FALSE !== strpos(SMS_HTTP_POST_TEMPLATE, '_RECIPIENTS_INTERNATIONAL')) {
            ...
            $cellcast_numbers = $intls;
        }
        $content = json_encode(array('from' => '61424284777', 'sms_text' => $cellcast_message, 'numbers' => $cellcast_numbers));
        $header = "" . ifdef('SMS_HTTP_HEADER_TEMPLATE', '');
        //             $header = $header . "\r\n" . "Content-Length: " . strlen($content) . "\r\n" . "Content-Type: application/x-www-form-urlencoded\r\n";
        ///HACK TO WORK WITH JSON FOR CELLCAST
